...
# create an executer with max 1 worker for GPU background tasks
...
def get_description(yt: video_utils.YoutubeDL, video_path: str, augmenter = None) -> str:
    """
...
def download_video(result, proxy_url):
    ...
    try:
      ...
      if result['length'] >= 6*60:
        ...
        end = min(result['length'], FIVE_MINUTES + MAX_VIDEO_LENGTH)
        bt.logging.info(f"Downloading video from 5m to 7m mark")
      ...
      if download_path:
        download_path_name = download_path.name
    
    except AttributeError as e:
      bt.logging.error(f"AttributeError in download_video: {e}")
      raise
    finally:
      if download_path:
        download_path.close()
      
    return download_path_name, result

@broker.task
def slice_video(download_path, result, num_videos):
    clip_paths = []
    if download_path:
      clip_path = None
      try:
          result['length'] = video_utils.get_video_duration(download_path)  # correct the length
          #bt.logging.info(f"Downloaded video {result['video_id']} ({min(result['length'], FIVE_MINUTES)}) in {time.time() - start} seconds")
          
          start = 0
          end = min(result['length'], MAX_VIDEO_LENGTH)
          bt.logging.info(f"Creating clip of video {result['video_id']} -- from {start}s to {end}s")
          clip_path = video_utils.clip_video(download_path, start, end)
          if clip_path is not None:
            clip_paths.append([clip_path.name, start, end])
            if clip_path:
              clip_path.close()
         
      except AttributeError as e:
          bt.logging.error(f"AttributeError in slice_video: {e}")
          raise
      finally:
...
async def search_and_embed_videos(original_query: str, query: str, num_videos: int, imagebind: ImageBind, start_function, augmenter) -> List[VideoMetadata]:
    ...
    try:
        ...
        results_list = []
        download_tasks = []
        # download our videos first in taskiq background
        for result in results:
            download_task = await download_video.kiq(result, proxy_url)
            download_tasks.append(download_task)
            results_list.append(result)
            if len(download_tasks) >= num_videos:
                break
        
        # Process downloads as they complete
        for download_future in asyncio.as_completed([task.wait_result() for task in download_tasks]):
            task_result = await download_future
            download_path = task_result.return_value[0]
            result = task_result.return_value[1]
            if download_path is not None:
              slice_task = await slice_video.kiq(download_path, result, num_videos)
              slice_result = await slice_task.wait_result()
              clips = slice_result.return_value
              if clips is not None and len(clips) > 0:
                for clip in clips:
                  ...
                  description = None
                  #description = get_description_from_subtitles(results_list[i], imagebind, clip_path, subtitles, query)
                  loop = asyncio.get_running_loop()
                  #description = await loop.run_in_executor(gpu_executor, get_description_from_subtitles, results_list[i], imagebind, clip_path, subtitles, query)
                  if description == "SKIPCLIP":
                      continue
                  if not description or description == "":
                      description = get_description(result, clip_path)
                  
                  #embeddings = imagebind.embed([description], [clip_path])
                  with open(clip_path, 'rb') as clip_binaryIO:
                    embeddings = await loop.run_in_executor(gpu_executor, imagebind.embed, [description], [clip_binaryIO])
                  os.remove(clip_path)
        ...
        bt.logging.error(f"Error in chain: {e}")
        # Log the stack trace
...
def get_subtitles(video_path: str):
    subtitles = ""
    subtitle_file = video_path.replace(".mp4", ".en.srt")
    if not os.path.exists(subtitle_file):
        subtitle_file = video_path.replace(".mp4", ".en.vtt")
...
def file_path_to_binary_io(file_path: str) -> BinaryIO:
    try:
